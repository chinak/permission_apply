...
- [docs/warehouse-transfer-prd.md](file://docs/warehouse-transfer-prd.md)
</cite>

## 更新摘要
**变更内容**
- 新增完整的部署基础设施，包括Linux自动化部署脚本、Nginx配置文件模板
- 新增Windows PowerShell一键部署脚本和开发环境启动脚本
- 新增详细的技术文档和产品需求文档
- 完善构建配置与静态资源缓存策略
- 增强故障排除指南和回滚机制
...
**更新** 新增了完整的部署基础设施，包括Linux和Windows双平台部署脚本、详细的Nginx配置模板以及技术文档支持。

## 项目结构
- 构建产物位于 dist/，包含入口 HTML 与按哈希命名的 JS/CSS 资源，适合长期缓存策略。
- 部署相关文件集中在 deploy/ 目录，包含部署脚本、Nginx 配置模板与使用说明。
- Windows平台支持通过 PowerShell 脚本进行一键部署。
- 开发环境启动脚本提供本地开发便利。
...
D["Windows部署脚本 deploy.ps1"] --> A
E["开发启动脚本 start.ps1"] --> F["本地开发环境"]
G["Vite 构建配置 vite.config.ts"] --> A
H["包管理与脚本 package.json"] --> D
I["PostCSS 配置 postcss.config.mjs"] --> G
J["工作区配置 pnpm-workspace.yaml"] --> H
K["应用入口 index.html"] --> L["React 入口 src/main.tsx"]
L --> M["应用根组件 src/app/App.tsx"]
M --> N["布局与导航 src/app/layout.tsx"]
O["技术文档 docs/warehouse-transfer-design.md"] --> P["产品需求 docs/warehouse-transfer-prd.md"]
```
...
- 部署脚本
  - **新增** Linux自动化部署脚本负责备份旧版本、复制新文件、设置权限、生成 Nginx 配置并重载服务。
  - **新增** Windows PowerShell一键部署脚本，自动构建并上传至服务器，最后尝试重载 Nginx。
  - **新增** 开发环境启动脚本，简化本地开发流程。
- Nginx 配置
  - **新增** 提供标准与默认站点两类配置模板，包含 gzip、静态资源缓存、SPA 路由回退、错误页映射与安全头等。
...
**更新** 新增了Windows平台的完整部署支持和开发环境管理。

```mermaid
graph TB
subgraph "开发机"
V["Vite 构建<br/>vite.config.ts"] --> D["dist 产物"]
S["开发启动脚本<br/>start.ps1"] --> Dev["本地开发环境"]
end
subgraph "服务器"
N["Nginx<br/>deploy/nginx.conf"] --> S["静态资源<br/>dist/*"]
P["Linux部署脚本<br/>deploy/deploy.sh"] --> N
PS["Windows部署脚本<br/>deploy.ps1"] --> D
PS --> N
end
U["用户浏览器"] --> N
```
...
### 开发环境启动脚本
**新增** 提供简化的本地开发环境启动流程，自动处理依赖安装、开发服务器启动和环境配置。

```mermaid
flowchart TD
Start(["启动开发环境"]) --> CheckNode["检查 Node.js 版本"]
CheckNode --> InstallDeps["安装项目依赖"]
InstallDeps --> StartDev["启动开发服务器"]
StartDev --> WatchFiles["监听文件变更"]
WatchFiles --> HotReload["热重载更新"]
HotReload --> OpenBrowser["自动打开浏览器"]
OpenBrowser --> Running(["开发环境就绪"])
```
...
### 技术文档支持
**新增** 项目包含详细的技术文档和产品需求文档，为部署和运维提供完整的业务背景和技术规范。

- 仓库迁移设计文档：详细说明系统架构设计和迁移策略
- 仓库转移产品需求文档：明确功能需求和业务逻辑

**章节来源**
- [docs/warehouse-transfer-design.md:1-100](file://docs/warehouse-transfer-design.md#L1-L100)
- [docs/warehouse-transfer-prd.md:1-100](file://docs/warehouse-transfer-prd.md#L1-L100)
...
Sh --> Dist
Sh --> Nginx["Nginx 服务"]
Start["start.ps1"] --> DevEnv["开发环境"]
Design["warehouse-transfer-design.md"] --> PRD["warehouse-transfer-prd.md"]
```
...
**章节来源**
- [deploy/deploy.sh:25-93](file://deploy/deploy.sh#L25-L93)
- [deploy.ps1:38-55](file://deploy.ps1#L38-55)
- [deploy/README.md:105-142](file://deploy/README.md#L105-L142)
- [start.ps1:1-50](file://start.ps1#L1-L50)

## 结论
本项目提供了完善的前端部署与运维方案：清晰的构建配置、可复用的 Nginx 模板、自动化与手动双通道部署脚本，以及 HTTPS 与缓存策略建议。**新增** 的Windows平台支持和开发环境管理进一步提升了部署效率和开发体验。遵循本文档可实现稳定、高效、可回滚的上线流程，并具备良好的扩展性与安全性。
...
```mermaid
flowchart TD
A["执行 start.ps1"] --> B["检查 Node.js 环境"]
B --> C["安装项目依赖"]
C --> D["启动开发服务器"]
D --> E["监听文件变更"]
E --> F["自动打开浏览器"]
