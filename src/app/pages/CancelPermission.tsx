...
import { Download, Search, Eye, ChevronDown, FileText, StickyNote, RefreshCw, User, CheckCircle2 } from 'lucide-react';
...
// 身份证占位图（front=人像面，back=国徽面）——内联 SVG，离线可靠渲染
      ...
      <Dialog open={isProcessOpen} onOpenChange={setIsProcessOpen}>
        <DialogContent className="!max-w-[65vw] w-full h-[90vh] p-0 border-none rounded-sm overflow-hidden bg-slate-50 flex flex-col gap-0">
          ...
          <div className="shrink-0 flex items-center gap-3 px-6 py-4 bg-white border-b border-slate-200 shadow-sm z-20">
            ...
            {processApp && (
              <span className="ml-3 text-xs text-slate-400 font-mono">{processApp.id}</span>
            )}
          </div>
          {/* Scrollable body */}
          <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-5">
            {processApp && (() => {
              ...
              const detail = {
                account: processApp.account,
                name: processApp.name,
                branch: processApp.branch,
                branchGroup: processApp.branchGroup,
                applyDate: processApp.applyDate,
                status: processApp.status,
              };
              ...
              const processStatus: string = processApp.status;
                        ...
                        <table className="w-full text-sm text-left">
                          ...
                          <tbody className="divide-y divide-slate-100 bg-white">
                            ...
                            <tr className="hover:bg-slate-50/50">
                              ...
                              <td className="py-3 px-4 text-slate-500 font-mono text-[13px]">{isSuccessDone ? `${processApp.processDate}:00` : '-'}</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                  ...
                  <section className="bg-white rounded-sm shadow-sm border border-slate-200">
                    ...
                    <div className="p-6">
                      {(() => {
                        ...
                        const status: string = processApp.status;
                        ...
                        const nodes: FlowNode[] = [
                          { label: '发起申请', actor: `${processApp.name} (客户)`, time: t(processApp.applyDate), state: 'done' },
                        ];

                        if (isSuccess) {
                          nodes.push(
                            { label: '营业部经办', actor: processApp.operator, time: t(processApp.processDate), state: 'done' },
                            { label: '营业部复核', actor: processApp.reviewer, time: t(processApp.lastCapTime), state: 'done' },
                            { label: '办理成功', actor: '系统', time: t(processApp.lastCapTime), state: 'done' },
                          );
                        } else if (atReview) {
                          nodes.push({ label: '营业部经办', actor: processApp.operator, time: t(processApp.processDate), state: 'done' });
                          if (isFail) {
                            nodes.push({ label: '营业部复核 (办理失败)', actor: processApp.reviewer, time: t(processApp.lastCapTime), state: 'fail', reason: processApp.remark });
                          } else {
                            nodes.push({ label: '营业部复核 (处理中)', actor: processApp.reviewer !== '-' ? processApp.reviewer : '等待处理', time: '-', state: 'current' });
                          }
                        } else {
                          // 经办阶段
                          if (isFail) {
                            nodes.push({ label: '营业部经办 (办理失败)', actor: processApp.operator, time: t(processApp.processDate), state: 'fail', reason: processApp.remark });
                          } else {
                            nodes.push({ label: '营业部经办 (处理中)', actor: processApp.operator !== '-' ? processApp.operator : '等待处理', time: '-', state: 'current' });
                          }
                        }
      ...
      <Dialog open={isApproveOpen} onOpenChange={setIsApproveOpen}>
        <DialogContent className="max-w-md p-0 border-none rounded-sm overflow-hidden bg-white">
          ...
          <div className="p-6 text-sm text-slate-600 leading-relaxed">
            ...
            {processApp && (
              <span className="block mt-2 text-xs text-slate-400 font-mono">流水号：{processApp.id}</span>
            )}
          </div>
          <DialogFooter className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 m-0 flex justify-end gap-3 sm:space-x-0">
